...
export function getComboboxButton(): HTMLElement | null {
	return document.querySelector(
		'button,[role="button"],[id^="headlessui-combobox-button-"]'
	)
}
...
export function assertLabelValue(element: HTMLElement | null, value: string) {
	...
	if (element.hasAttribute("aria-labelledby")) {
		...
		expect(ids.map((id) => document.getElementById(id)?.textContent).join(" ")).toEqual(
			value
		)
		return
	}
...
export function assertTabs(
	...
	try {
		...
		let activeTab = Array.from(list.querySelectorAll('[id^="headlessui-tabs-tab-"]'))[
			active
		]
		let activePanel = panels.find(
			(panel) => panel.id === activeTab.getAttribute("aria-controls")
		)
